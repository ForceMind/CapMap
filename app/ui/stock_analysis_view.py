...
def render_stock_analysis_view(origin_df):
    ...
    with col1:
        ...
        #    强制添加一个选项，允许用户"回车"确认查询
             ...
             # 触发自动更新列表逻辑 (如果完全没找到匹配)
             if len(options_map) == 1:
                # 只有这一个手动选项，意味着本地列表可能缺失
                # 我们在下面的 selectbox 显示之后，不立即刷新，而是给个提示或自动刷
                pass
        ...
        if options_map:
            ...
            # 搜索无结果情况 (search_text not empty, but filtered_df is empty)
            st.warning("⚠️ 未在本地列表找到匹配股票")
            if st.button("🚀 没找到？点此立即从网络更新列表"):
                with st.spinner("正在全量同步最新A股列表，请稍候..."):
    ...
    with col3:
        ...
        if st.checkbox("更新列表", value=not options_map and search_text, help="如果搜不到股票，勾选此项强制拉取最新列表，或在找不到时自动触发"):
             with st.spinner("正在同步A股列表 (约需5-10秒)..."):
                 get_all_stocks_list(force_update=True)
             st.success("列表已更新，请重新搜索")
             st.rerun()
